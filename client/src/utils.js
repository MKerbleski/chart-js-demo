...
export const getSomeRGBAColors = (number, opacity=1) => {
...
export const getRandomColor = (opacity) => {
    return `rgba(${Math.floor(Math.random() * Math.floor(255))}, ${Math.floor(Math.random() * Math.floor(255))}, ${Math.floor(Math.random() * Math.floor(255))}, ${opacity} )`
}